...
- 列表名称下方用 `Chat · Resp · Msg` 简称展示支持的协议，悬停可查看完整名称；状态标签使用 `Chat / Responses / Messages`。操作列保持单行：编辑、设为当前、停用；“设为当前”菜单列出已配置的协议，当前绑定用勾标记。该 Provider 已配置的协议全都绑定为当前时隐藏菜单；协议被切换到其他 Provider 或新增支持协议后重新显示。停用继续使用确认气泡。模型探测只在新建和编辑表单中操作。
- 搜索词、协议和状态先保存在表单 Signal 中；点击“查询”或按 Enter 后，Topcoat Shard 只更新 Provider 列表区域。“重置”和空结果的“清除筛选”也局部更新。带筛选参数的 `/ui` 链接仍可直接打开；页内查询不改地址栏，手动刷新会恢复 URL 指定的筛选条件。
